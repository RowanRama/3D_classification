...
class BoxFilter{
  ...
  private:
    ...
    void bbox_cb (const classification::bbox_2d::ConstPtr& msg) {
      ...
      float w = msg->width.data/2;
      min_ = Eigen::Vector4f(msg->origin.position.x - w, msg->origin.position.y - w, 0, 1.0 );
      max_ = Eigen::Vector4f(msg->origin.position.x + w, msg->origin.position.y + w, 999, 1.0 );
    };
